...
def free_bacon(score):
    ...
    while True:
        pi //= 10
        if (pi >= 10 ** score) and (pi < 10 ** (score+1)):
            break
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            this_score = score0 - last_score
            if this_score > running_high:
                print(f"{this_score} point(s)! The most yet for Player {who}")
                return announce_highest(who, score0, this_score)
            else:
                return announce_highest(who, score0, running_high)
        else:
            if who == 1:
                this_score = score1 - last_score
                if this_score > running_high:
                    print(f"{this_score} point(s)! The most yet for Player {who}")
                    new_high = this_score
                    return announce_highest(who, score1, this_score)
                else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    get_averaged_score = make_averaged(roll_dice, trials_count)
    max_averaged_score = 0
    best_dice_num = 0

    for i in range(1, 11):
        averaged_score = get_averaged_score(i, dice)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score > opponent_score:
        return extra_turn_strategy(score, opponent_score, 3, 1)
    else:
        return extra_turn_strategy(score, opponent_score, 3, 4)
    # END PROBLEM 12
