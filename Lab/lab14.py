    ...
    if t.is_leaf():
        ...
        if len(t.branches) == 2:
            ...
            if b1.label < b2.label:
                t.branches.remove(b2)
            else:
                t.branches.remove(b1)
...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    #本题参考https://github.com/yngz/cs61a/blob/master/lab/lab13/lab13_extra.py#L4
    def helper(xs, n=0):
        if len(xs) == 0:
            if abs(n) <= d:
                return 1
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.payable_to is self.holder:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        edits, cost = "+[" + code_char + "]" + edits, 1 + cost
        possibilities.append((edits, cost))
        # Delete
        edits, cost = helper_align(skeleton_idx + 1, code_idx)
        edits, cost = "-[" + skel_char + "]" + edits, 1 + cost
        possibilities.append((edits, cost))
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Implementation without foldr and foldl
